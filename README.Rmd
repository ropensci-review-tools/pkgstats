...
libraries on various operating systems are described in a separate vignette.

Once you've installed these libraries, the package itself can be installed
with either of the following two lines:
...
The package includes a function to ensure your local installations of `universal-ctags`
and `global` work correctly. Please ensure you see the following prior to
proceeding:
...
function](https://ropenscilabs.github.io/pkgstats/reference/pkgstats.html) to
a single line with `r ncol (s)` entries, represented as a `data.frame` with one
row and that number of columns, enabling multiple packages to be compared by
binding rows together. The following lists describe these `r ncol (s)`
