...
#' file.
#' @param num_cores Use specified number of cores for parallel processing,
#' defaulting to one half of all available cores. If set to `1L`, calculate as
#' single job, displaying progress information on screen. See note.
#' @param results_path Path to save intermediate files generated by the
#' `chunk_size` parameter described above.
#'
#' @note Parallel processing may be unreliable, as each job requires writing and
...
pkgstats_from_archive <- function (path,
    ...
    if (nfiles > 0) {
        ...
        if (is.null (num_cores)) {
            ...
            if (!(class (num_cores) [1] == "integer" &
                  length (num_cores) == 1L))
                stop ("num_cores must be a single integer value")
        }
