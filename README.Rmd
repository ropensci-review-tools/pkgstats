...
1. Numbers of lines of code, documentation, and white space in each directory and language
...
this metric is provided as a relative indication of the general readability of
...
[`pkgstats_summary()`
function](https://ropensci-review-tools.github.io/pkgstats/reference/pkgstats_summary.html)
aims to return as many usable statistics as possible in order to flexibly
allow higher-level statistics to be derived through combination and
aggregation. The following lists describe these `r ncol (s)` statistics (not in
the order in which they actually appear), with variable names in parentheses
...
- Measures of relative white space in each sub-directory (`rel_space_R`,
  ...
  (`rel_space`).
- The number of spaces used to indent code, noting that the tools used to
  extract the `object` and `network` tables described below do not work on code
  with tab-indentation, and `pkgstats` thus converts all tab characters to
  a fixed number of spaces (with a default of 2; `indentation`).
