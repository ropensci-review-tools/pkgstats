...
fn_var_call_graph_r <- function (fns, fn_vars, path) {
    ...
    for (f in unique (fns$file)) {
        ...
        pd <- controlParseData (file = f_full)
...
#' Error-controlled version of getParseData
#'
#' getParseData can fail even with enforced encoding because of multi-byte
...
#' @return contents of `getParseData(x)`
#' @noRd
controlParseData <- function (file) {
